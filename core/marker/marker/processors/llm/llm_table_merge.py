...
"""
    ...
    def process_rewriting(self, document: Document, blocks: List[Block]):
        ...
        for i in range(1, len(blocks)):
            ...
            print("Merging Tables", start_block.block_id, curr_block.block_id)

            # Merge the cells and images of the tables
            direction = response["direction"]
            if not self.validate_merge(children, children_curr, direction):
                print("Invalid Merge", start_block.block_id, curr_block.block_id)
                start_block = curr_block
                continue

            print("Valid Merge",start_block.block_id, curr_block.block_id)
    ...
    def validate_merge(self, cells1: List[TableCell], cells2: List[TableCell], direction: Literal['right', 'bottom'] = 'right'):
        if direction == "right":
            ...
            print("Direction Right, cells1_row_count", cells1_row_count, "cells2_row_count", cells2_row_count)
            ...
            print("Direction bottom, cells1_col_count", cells1_col_count, "cells2_col_count", cells2_col_count)

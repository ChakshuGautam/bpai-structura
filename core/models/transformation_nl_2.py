...
# V2: Added new character pairs based on error analysis
similar_char_groups = {
    ...
    'प': ['य', 'घ', 'क', 'र'],
    'घ': ['प', 'ध', 'छ'],  # V2: Added छ (छत→घत confusion)
    'ध': ['घ'],
    'म': ['थ'],
    'थ': ['म'],
    'क': ['फ', 'प', 'र'],
    'फ': ['क'],
    'ग': ['य', 'भ'],  # V2: Added भ (भर→गर confusion)
    'в': ['ल'],
    'न': ['ब', 'ल'],
    'ब': ['न'],  # Added अ for अब/जब case
    'ठ': ['भ'],
    'भ': ['ठ', 'ग'],  # V2: Added ग (भर→गर confusion)
    'ट': ['त'],
    'त': ['ट'],
    'श': ['स'],
    'स': ['श'],
    ...
    # V2: NEW ADDITIONS based on error analysis
    'छ': ['घ', 'ख'],  # छत→घत, छत→खत confusions
    'ख': ['छ', 'क'],  # खत→छत confusion
    'च': ['य'],       # Observed च/य confusion
}
...
def remove_consecutive_duplicates(word_evaluations):
    """
    V2: NEW FUNCTION
    Remove consecutive duplicate words that Gemini sometimes generates.
    Examples: "पढ़ पढ़" → "पढ़", "खरही खरही" → "खरही"
    """
    if not word_evaluations or len(word_evaluations) < 2:
        return word_evaluations

    i = 0
    while i < len(word_evaluations) - 1:
        current_word = clean_word(word_evaluations[i].get('transcribed_word', ''))
        next_word = clean_word(word_evaluations[i+1].get('transcribed_word', ''))

        # Check if consecutive words are identical and non-empty
        if current_word and current_word == next_word:
            # Mark the duplicate as a match and clear its transcribed_word
            word_evaluations[i+1]['transcribed_word'] = None
            word_evaluations[i+1]['match'] = True
            word_evaluations[i+1]['reason_diff'] = (
                word_evaluations[i+1].get('reason_diff', '') +
                f" [Duplicate word removed: consecutive '{current_word}']"
            ).strip()
...
def transform_word_evaluations_nl(word_evaluations):
    ...
    V2: Added duplicate removal and expanded character similarities.
    """

    # V2: NEW - Remove consecutive duplicates first
    word_evaluations = remove_consecutive_duplicates(word_evaluations)
